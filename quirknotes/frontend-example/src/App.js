...
function App() {
  ...
  const deleteNote = async (entry) => {
    try {
      ...
      if (!response.ok) {
        ...
        alert(`Failed to delete note: ${response.status}`);
        throw new Error(`HTTP status ${response.status}`);
      }
  
      // If successful, remove the note from the state
  ...
  const deleteAllNotes = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        alert(`Failed to delete all notes: ${response.status}`);
        return;
      }
      // If successful, clear the notes from the state
      deleteAllNotesState();
    } catch (error) {
      console.error("Fetch function failed:", error);
  ...
  const patchNoteState = (_id, title, content) => {
    setNotes((prevNotes) => prevNotes.map(note => note.id === _id ? { ...note, title, content } : note));
